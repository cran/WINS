iptw_covipcw.win.stat<-function(df, Z_t_trt, Z_t_con,iptw.weight,n_total,arm.name = c(1,2),id_trt,id_con,
  ...
  arm = df$arm
  weight.trt = iptw.weight[arm==arm.name[1]]
  weight.con = iptw.weight[arm==arm.name[2]]
  ...
  weighted_win_status = iptw_covipcw.adjusted.KL(win_status = win_status, trt = trt,con = con, trt_con = trt_con,
                                              Z_t_trt, Z_t_con, priority = priority, n_ep = n_ep, ep_type = ep_type,
                                              weight.trt = weight.trt, weight.con = weight.con)
  KL = weighted_win_status$KL

  #### number of patients per stratum
  df_trt = data.frame(stratum = trt$stratum, wt = weight.trt)
  N_trt = aggregate(x = list("wt" = df_trt$wt), by = list("stratum" = df_trt$stratum), FUN = sum)[,2]

  df_con = data.frame(stratum = con$stratum, wt = weight.con)
  N_con = aggregate(x = list("wt" = df_con$wt), by = list("stratum" = df_con$stratum), FUN = sum)[,2]
  ...
  summary_ep = apply(weighted_win_status$status_KL, 2, func<-function(x){
    temp1 = aggregate(x, by=list(trt_con$stratum), FUN = sum)
    temp2 = aggregate(x, by=list(trt_con$stratum), FUN = sum)
    temp_res = cbind(temp1,temp2[,2]/(N_trt*N_con)); colnames(temp_res) = c("Stratum", "Count", "Proportion")
    return(temp_res)
  })
