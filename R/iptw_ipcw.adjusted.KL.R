iptw_ipcw.adjusted.KL<-function(win_status, trt, con, trt_con, priority, n_ep, ep_type, weight.trt, weight.con){
  ...
  for(l in n_ep){
    ...
    if(ep_type[ind.prior]=="tte"){
      ...
      csurv1.fit = survival::survfit(surv_trt~1,type='kaplan-meier',weights=weight.trt)
      ...
      csurv2.fit = survival::survfit(surv_con~1,type='kaplan-meier',weights=weight.con)
